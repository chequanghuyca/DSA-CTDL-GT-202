...
public:
	...
	int countTwoChildrenNode() { return countTwoChildren(this->root); }
	int countTwoChildren(Node* root) {
		int count;
		if (root == NULL) { return 0; }
		count = countTwoChildren(root->pLeft) + countTwoChildren(root->pRight);
		if (root->pLeft != NULL && root->pRight != NULL) { return count + 1; }
		return count;
	}
};
